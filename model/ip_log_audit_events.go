...
func ListIPLogAuditEvents(ip string, startIdx, limit int) ([]IPLogAuditEvent, int64, error) {
	...
	var logs []Log
	if err := query.Select("id, created_at, type, username, model_name, token_name, quota, prompt_tokens, completion_tokens, use_time, request_id").
	...
	for _, log := range logs {
		events = append(events, IPLogAuditEvent{
			...
			UseTime: log.UseTime, RequestID: log.RequestId,
		})
	}
